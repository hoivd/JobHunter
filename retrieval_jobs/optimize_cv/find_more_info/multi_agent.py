...
# Thêm vào sys.path ở vị trí đầu tiên (ưu tiên trước)
...
from .nodes.planning_mission.analysic_per_info import AgentAnalysisPerInfo
from .nodes.planning_mission.jd_extraction import AgentJDExtraction
from .nodes.planning_mission.plan_find_info import AgentPlanFindInfo 
from .nodes.planning_mission.complete_cv import AgentCompleteCV
from .find_info import MissionAgentNode 
...
class AgentSystem:
    ...
    def _init_graph(self):
        ...
        self.workflow.add_node("agent_jd_extraction", self.agent_jd_extraction)
        self.workflow.add_node("agent_analysis_per_info", self.agent_analysis_per_info)
        self.workflow.add_node("agent_plan_find_info", self.agent_plan_find_info)
        self.workflow.add_node("agent_find_info", self.agent_find_info) 
        self.workflow.add_node("agent_complete_cv", self.agent_complete_cv)



        self.workflow.set_entry_point("agent_jd_extraction")

        self.workflow.add_edge("agent_jd_extraction","agent_analysis_per_info")
        self.workflow.add_edge("agent_analysis_per_info", "agent_plan_find_info")
        self.workflow.add_edge("agent_plan_find_info", "agent_find_info")
        self.workflow.add_edge("agent_find_info", "agent_complete_cv")
        # self.workflow.add_edge("agent_plan_find_info", "agent_complete_cv")
        self.app = self.workflow.compile()
    

    def simple_router(self, state: State) -> str:
        step = state.get("current_step")

        if step == "agent_jd_extraction":
            return "agent_analysis_per_info"
        elif step == "agent_analysis_per_info":
            return "agent_plan_find_info"
        elif step == "agent_plan_find_info":
            return "END"
        else:
            return "END"
